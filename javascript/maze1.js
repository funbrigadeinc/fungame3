...
function component(width, height, color, x, y, type) {
  ...
  this.crashWith = function(obst){
    ...
    //document.getElementById('console5').innerHTML= condition;
    /*var crash = true;
    if ((mybottom <= obsttop) || (mytop >= obstbottom) || (myright <= obstleft) || (myleft >= obstright)) {
        crash = false;
    }*/
    /*if (crash){
        if(myright >= obstleft ){ hitDirection="right"; }
        else if(myleft <= obstright){ hitDirection="left";  }
        else if(mybottom >= obsttop){ hitDirection="top";   }
        else if(mytop <= obstbottom){ hitDirection="bottom";}
    }*/
